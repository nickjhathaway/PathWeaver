...
int WeaverRunner::BamExtractPathwaysFromRegion(
	...
	bool noTrim = false;
	bool force_chrom_renaming = false;
	...
	setUp.setOption(force_chrom_renaming, "--force_chrom_renaming", "when renmaing chromosomes, by default will only rename if the new key is in bam header, this forces the renaming key no matter what");
	...
	VecStr chroms_in_bam_header;
	{
		...
		reader.Open(setUp.pars_.ioOptions_.firstName_.string());
...
//	}
	...
	if(!chromKey.empty()){
		brInvestor.chromRenamingKey_ = BamRegionInvestigator::processChromRenaming(chromKey);
	}
  ...
  std::function<void(const HaploPathFinder::ExtractParams &)> extractPathway;
  extractPathway = [&meta, &regionsQueue, &jLog, &refDir,
          ...
          &keepTemporaryFiles, &brInvestor, chroms_in_bam_header](const HaploPathFinder::ExtractParams &inputPars) {
      ...
      try {
        ...
        for (const auto &reg: regInfo) {
          regionsUsed.emplace_back(reg->region_);
        	for(const auto & key : brInvestor.chromRenamingKey_) {
        		if(std::string::npos != regionsUsed.back().chrom_.find(key.first)) {
        			auto new_chrom_name = njh::replaceString(regionsUsed.back().chrom_, key.first, key.second);
        			if (njh::in(new_chrom_name, chroms_in_bam_header)) {
        				regionsUsed.back().chrom_ = new_chrom_name;
        			}
        		}
        	}
        }
...
//							auto perBaseCoverage = bases/static_cast<double>(lenSum);
        ...
        logValue = PWRes.log_;
        auto outputOpts = SeqIOOptions::genFastaIn(
                njh::files::make_path(regionDir, sampName, "output_aboveCutOff.fasta"), true);
...
//									SeqOutput::write(finalSeqsOnClusters, finalKeptSeqOpts);
              ...
              if (reDetermineReadCounts) {
                ...
                aligner alignerObj(maxLen, gapScoringParameters(5, 1, 5, 1, 5, 1), substituteMatrix(2, -2),
                                   countEndGaps);
                ...
                auto increaseMapCounts = [&unmappable, &indeterminate, &uniqueSeqs, &multiMappingSeqs, &refInfos, &finalSeqsOnClusters, &alignerObj,
                  ...
                  for (const auto refPos: iter::range(finalSeqsOnClusters.size())) {
                    ...
                    if (reMappingSeqInfo.compareKmers(refInfos[refPos]).second < matchingKmerCutOff &&
                        reMappingSeqInfo.compareSubKmersToFull(refInfos[refPos], 0, subSize).second <
                        matchingKmerCutOff &&
                        reMappingSeqInfo.compareSubKmersToFull(refInfos[refPos], len(reMapSeq) - subSize,
                                                               subSize).second < matchingKmerCutOff) {
                      continue;
                    }
                  ...
                  if (bestRefs.size() == 1) {
                    ...
                    for (const auto seqAlignPos: iter::range(alignerObj.alignObjectA_.seqBase_.seq_.size())) {
                      if (alignerObj.alignObjectA_.seqBase_.seq_[seqAlignPos] != '-' &&
                        ...
                        baseCoverrage[finalSeqsOnClusters[bestRefs.front()].seqBase_.name_][alignerObj.getSeqPosForAlnAPos(
                                seqAlignPos)][reMapSeq.name_] = 1;
                      }
                    }
                    ...
                    for (const auto &refPos: bestRefs) {
                      ...
                      //												}
                      bool pass = true;
                      uint32_t queryStart = alignerObj.getSeqPosForAlnAPos(
                              alignerObj.alignObjectB_.seqBase_.seq_.find_first_not_of("-"));
                      uint32_t queryStop = alignerObj.getSeqPosForAlnAPos(
                    ...
                    if (matchingRefs.size() == 1) {
                      ...
                      for (const auto seqAlignPos: iter::range(alignerObj.alignObjectA_.seqBase_.seq_.size())) {
                        if (alignerObj.alignObjectA_.seqBase_.seq_[seqAlignPos] != '-' &&
                            alignerObj.alignObjectB_.seqBase_.seq_[seqAlignPos] != '-') {
                          baseCoverrage[finalSeqsOnClusters[matchingRefs.front()].seqBase_.name_][alignerObj.getSeqPosForAlnAPos(
                                  seqAlignPos)][reMapSeq.name_] = 1;
                        }
                      }
                ...
                for (const auto &multi: filteredMultiMappingSeqs) {
                  ...
                  for (const auto &pos: refPoss) {
                    ...
                    finalTotals[finalSeqsOnClusters[pos].seqBase_.name_] +=
                            namesSet.size() * (uniqueMapFrac[finalSeqsOnClusters[pos].seqBase_.name_] / totalFrac);
                    for (const auto &reMapSeq: multi.second) {
                      alignerObj.alignCacheGlobal(finalSeqsOnClusters[pos].seqBase_, reMapSeq);
                      for (const auto seqAlignPos: iter::range(alignerObj.alignObjectA_.seqBase_.seq_.size())) {
                        if (alignerObj.alignObjectA_.seqBase_.seq_[seqAlignPos] != '-' &&
                            alignerObj.alignObjectB_.seqBase_.seq_[seqAlignPos] != '-') {
                          baseCoverrage[finalSeqsOnClusters[pos].seqBase_.name_][alignerObj.getSeqPosForAlnAPos(
                                  seqAlignPos)].emplace(reMapSeq.name_,
                                                        uniqueMapFrac[finalSeqsOnClusters[pos].seqBase_.name_] /
                                                        totalFrac);
                        }
                      }
                    }
                ...
                for (const auto &bCov: baseCoveragesFinal) {
                  ...
                  if (bCov.second.size() > 60) {
                    ...
                    meanCoverage = vectorMean(
                            getSubVector(bCov.second, 25, finalSeqsOnClusters[bCov.first].seqBase_.seq_.size() - 50));
                  } else {
              ...
              if (filterOnBaseCoverage) {
                ...
                if (finalSeqsOnClusters.size() == offCount) {
                  std::stringstream ss;
                  ss << __PRETTY_FUNCTION__ << " error, no final seqs above mean base coverage cut off of "
                     << baseCoverageCutOff << "\n";
                  throw std::runtime_error{ss.str()};
                } else {
                  auto finalSeqBellowMeanBaseCovOpts = SeqIOOptions::genFastaOut(
                          njh::files::make_path(finalInfoRegionDir, "seqBelowCoverageCutOff.fasta"));
...
//									outFinalInfo << "\n";
              ...
              for (auto &clus: finalSeqsOnClusters) {
                ...
                clus.seqBase_.name_ = regionName + "." + leftPadNumStr<uint32_t>(count, finalSeqsOnClusters.size()) +
                                      metaName.createMetaName();
                clus.updateName();

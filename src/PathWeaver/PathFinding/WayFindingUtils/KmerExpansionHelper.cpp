...
KmerExpansionHelper::CondensedSeq::CondensedSeq(const std::string & seq){
...
KmerExpansionHelper::KmerExpansionHelper(const std::unordered_map<std::string, motif>& tandemMots,
...
void KmerExpansionHelper::addToTandemLocs(
...
void KmerExpansionHelper::addToGlobalTandems(const std::string & seq,
		uint32_t klength,
...
std::unordered_map<std::string, std::vector<StartStopTanPos>> KmerExpansionHelper::processForTandems(const std::string & seq,
		const std::unordered_map<std::string, motif>& tandemMots,
		std::unordered_map<std::string, std::unordered_map<std::string, motif>> & tandemsAltMots,
		uint32_t klength){
	//single
...
std::vector<KmerPathwayGraph::StartEndPos> KmerExpansionHelper::processTandemLocsIntoPositions(const std::string & seq,
		 std::unordered_map<std::string, std::vector<StartStopTanPos>> tandemLocations){
	std::vector<KmerPathwayGraph::StartEndPos> ret;
	...
	for(auto & tloc : tandemLocations){
		...
		std::vector<KmerPathwayGraph::StartEndPos> locsForCurrent;
...
std::vector<KmerPathwayGraph::StartEndPos> KmerExpansionHelper::getPositionsForHomopolymers(const std::string & seq,
		const uint32_t klength){

	std::vector<KmerPathwayGraph::StartEndPos> positions;
	CondensedSeq testC(seq);
	uint32_t seqPosition = 0;
	for(const auto pos : iter::range(testC.base_.size())){
		if(testC.count_[pos] >= klength){
			positions.emplace_back(KmerPathwayGraph::StartEndPos(seqPosition, seqPosition + testC.count_[pos]));
		}
...
std::vector<KmerPathwayGraph::StartEndPos> KmerExpansionHelper::getExpansionPositions(const std::string & seq){
	...
	auto mergedPositions = KmerPathwayGraph::StartEndPos::merge(expansionLocs);
	if(!keepPositionsSmallerThanKlen_){
		std::vector<KmerPathwayGraph::StartEndPos> keep;
...
uint32_t KmerExpansionHelper::tandemMismatchCutOff = 0;
uint32_t KmerExpansionHelper::tandemRepeatNumbercutOff = 1;

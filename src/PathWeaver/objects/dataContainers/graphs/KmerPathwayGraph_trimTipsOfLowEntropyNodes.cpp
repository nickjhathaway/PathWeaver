/*
 * KmerPathwayGraph_trimTipsOfLowEntropyNodes.cpp
 *
...
bool KmerPathwayGraph::trimEdgesNodeTipsWithLowEntropy(const readVecTrimmer::TrimEdgesByLowEntropyPars & pars){
...
bool KmerPathwayGraph::trimTipsOfLowEntropyNodes(double entropyCutOff){
	std::vector<std::shared_ptr<node>> nodesToProcess;
	for(const auto & n : nodes_){
		if(n->on_ && (n->tailless() || n->headless())){
			nodesToProcess.emplace_back(n);

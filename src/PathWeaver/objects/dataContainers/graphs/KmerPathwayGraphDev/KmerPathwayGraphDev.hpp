...
class KmerPathwayGraphDev {
public:
	KmerPathwayGraphDev(uint32_t klen);
	KmerPathwayGraphDev(uint32_t klen, uint32_t occurenceCutOff);
	uint32_t klen_;
	...
	public:
		...
		std::string k_;
		std::string nameUid_; //uid by name
		uint32_t nodeUid_; //actual position in the vector<node> in the graph
...
//		std::string uid_;
		...
		std::shared_ptr<node> getHeadNode(const std::string & nameUid) const;
		std::shared_ptr<node> getTailNode(const std::string & nameUid) const;

		std::shared_ptr<node> getHeadNode(const uint32_t nodeUid) const;
		std::shared_ptr<node> getTailNode(const uint32_t nodeUid) const;
	...
	public:
		...
		std::string createUidWithNameUid() const;
		std::string createUidWithNodeUid() const;
	};
  ...
  bool hasSelfPointingPaths();
	...
	void writeRectangleWithEstimatedCovDot(std::ostream & out, KmerPathwayGraphDev & estimatingCovGraph, bool noLabels = false) const;
	void writeRectangleWithEstimatedCovDotByGroup(const OutOptions & prefixPath, KmerPathwayGraphDev & estimatingCovGraph, uint32_t groupSizeCutOff = 2, bool noLabels = false) const;
	...
	bool collapseOneBaseIndelsNodes(KmerPathwayGraphDev & estimatingCovGraph);
	//bool collapseOneBaseIndelsNodes3Nodes(KmerPathwayGraphDev & estimatingCovGraph);
	...
	KmerPathwayGraphDev copyGraph() const;
